...
fn bench_inflation(c: &mut Criterion) {
    ...
    // InflationLayer via LayeredGrid2d — linear curve (typical)
    c.bench_function("inflation_layer_linear_typical", |b| {
        let info = MapInfo {
            width: 256,
            height: 256,
            resolution: res_005,
        ...
        let inflation_layer = InflationLayer::linear(0.5);
    ...
    // InflationLayer with Inscribed curve (cost fn overhead)
    c.bench_function("inflation_layer_inscribed_typical", |b| {
        ...
        let positions: Vec<UVec2> = (0..256)
            ...
            .collect();
        let static_layer = StaticLethalsLayer { positions, info };
        let inflation_layer = InflationLayer::inscribed(0.5, 2.0, 1.0);
        let mut layered = LayeredGrid2d::new(info, COST_FREE, false);
        layered.add_layer(Box::new(static_layer));
        layered.add_layer(Box::new(inflation_layer));
        let robot = Pose2 {
